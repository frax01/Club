...
class TabMatchPage extends StatefulWidget {
  const TabMatchPage({super.key, required this.document});
...
class _TabMatchPageState extends State<TabMatchPage> {
  ...
  Widget build(BuildContext context) {
    ...
    var match = _matchesData.firstWhere((match) => widget.document['soccer_class'] == match['team'], orElse: () => <String, dynamic>{});
    print('match: $match');

    if (match != null) {
      _fetchWeatherData(match['team'], match['date']);
    }
    print(_matchesData.length);
      return Scaffold(
    body: SingleChildScrollView(
      child: match != null ? Padding(
          ...
          children: [
            SizedBox(
              ...
              child: Card(
                ...
                child: Center(
                  child: Text(match['team'], style: const TextStyle(fontSize: 18.0, fontWeight: FontWeight.bold)),
                ),
              ),
            ),
            Image.asset(
            ...
            ElevatedButton(
              onPressed: () {
                if (_res1Controller.text.isNotEmpty && _res2Controller.text.isNotEmpty) {
                  final opponent = match['opponent'];
                  final teams = opponent.split(' vs ');
                  final team1 = teams[0];
                  final team2 = teams[1];

                  _updateFootballCalendar(team1, team2, _res1Controller.text, _res2Controller.text);
                }
...
//    var heightScreen = size.height;
//    
//    var initialPage = _matchesData.indexWhere((match) => widget.document['soccer_class'] == match['team']);
//    print('initial: $initialPage');
